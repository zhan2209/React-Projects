  ...
  if (err)
    ...
    db.authenticate(dbUser, dbPassword, function(err,result){
      if(err)
        ...
        router.post('/searchcars', function(req,res){
          if(req.body.search === '') {
            db.collection(carColl).find({}).toArray(function(err, results) {
              if (err) {
                throw err;
              }
              else {
            ...
            db.collection(carColl).find({ $text : {$search : req.body.search}}).toArray(function(err, results) {
              if (err) {
                throw err;
              }
              else {
        ...
        router.post('/editcar', function(req,res){
          db.collection(carColl).update(
            { "_id" : new mongodb.ObjectID(req.body.id) },
            { $set : {
              car: {
                brand: req.body.car.brand,
                model: req.body.car.model,
                miles: req.body.car.miles,
                year: req.body.car.year,
                email: req.body.car.email,
                location: req.body.car.location,
                loc_loading: false,
                address: req.body.car.address,
        ...
        router.post('/deletecar', function(req,res){
          db.collection(carColl).remove({"_id": new mongodb.ObjectID(req.body.id) });
          res.send({status: 'success'});
        });

        router.post('/getallcars', function(req,res){
          db.collection(carColl).find({}).toArray(function(err, results) {
            if (err) {
              throw err;
            }
        ...
        router.post('/getcars', function(req,res){
          db.collection(carColl).find({"username": req.body.username}).toArray(function(err, results) {
            if (err) {
              throw err;
            }
            else {
        ...
        router.post('/sellcar', function(req,res){
          db.collection(carColl).insert(req.body);
          res.send({status: 'success'});
        });
